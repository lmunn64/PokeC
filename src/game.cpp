...
pokemon_t spawnPokemon(int id){ //if 0, random pokemon is generated, else it is set to the given pokemon_id
    ...
    std::vector possibleMoves = getMoves(newPokemon, 1);
    ...
    newPokemon->iv_hp = rand() % 16;
    newPokemon->iv_attack = rand() % 16;
    newPokemon->iv_defense = rand() % 16;
    newPokemon->iv_special_attack = rand() % 16;
    newPokemon->iv_special_defense = rand() % 16;
    newPokemon->iv_speed = rand() % 16;

    //get base for each stat
    newPokemon->base_hp = pokemonStatsVector.at((speciesId * 6) - 6).base_stat;
    newPokemon->base_attack = pokemonStatsVector.at((speciesId * 6) - 5).base_stat;
    newPokemon->base_defense = pokemonStatsVector.at((speciesId * 6) - 4).base_stat;
    newPokemon->base_special_attack = pokemonStatsVector.at((speciesId * 6) - 3).base_stat;
    newPokemon->base_special_defense = pokemonStatsVector.at((speciesId * 6) - 2).base_stat;
    newPokemon->base_speed = pokemonStatsVector.at((speciesId * 6) - 1).base_stat;

    //set all stats for new pokemon
    newPokemon->growthRate = pokemonSpecVector.at(speciesId - 1).growth_rate_id;
    newPokemon->expNeeded = experienceVector.at(((newPokemon->growthRate - 1)* 100 + lvl) - 1).experience;
    newPokemon->expCurrent = 0;

    newPokemon->baseExp = pokemonCSVVector.at(speciesId - 1).base_experience;
    newPokemon->maxHp = getPokemonStat(lvl, newPokemon->iv_hp, newPokemon->base_hp, 1);

    newPokemon->hp = getPokemonStat(lvl, newPokemon->iv_hp, newPokemon->base_hp, 1);
    newPokemon->attack = getPokemonStat(lvl, newPokemon->iv_attack, newPokemon->base_attack, 0);
    newPokemon->defense = getPokemonStat(lvl, newPokemon->iv_defense, newPokemon->base_defense, 0);
    newPokemon->special_attack = getPokemonStat(lvl, newPokemon->iv_special_attack, newPokemon->base_special_attack, 0);
    newPokemon->special_defense = getPokemonStat(lvl, newPokemon->iv_special_defense, newPokemon->base_special_defense, 0);
    newPokemon->speed = getPokemonStat(lvl, newPokemon->iv_speed, newPokemon->base_speed, 0);
    newPokemon->shiny = 0;
...
void createPlayer(terrain_type_t screen[21][80], int directionFrom){
    ...
    if(world[200][200]->NPC[0].trainer_pokemon[0].level == 0){
        ...
        pc->pokedollars = world[200][200]->NPC[0].pokedollars;
    }
...
void createMap(){
    ...
    world[currWorldRow][currWorldCol]->x = currWorldCol;
    world[currWorldRow][currWorldCol]->y = currWorldRow;
...
int movePC(character_t *t, char ch){ //if this returns a 0, then no gate traversal, if 1 then gate traversal north, if 
    ...
    if(ch == 'k' || ch == '8'){ // up
        ...
        if(world[currWorldRow][currWorldCol]->screen[minY-1][minX] == ter_gate){
            int sound = loadSound("music/SFX_SNARE_7.wav");
            playSound(sound);
            usleep(10000);
            return 1;
        }
...
int main(int argc, char *argv[]){
    ...
    int song = loadMusic("music/BG.mp3");
    aButton = loadSound("music/SFX_A_BUTTON.wav");
    playMusic(song);
